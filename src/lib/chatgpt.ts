...
async function main() {
  ...
  try {
    ...
    If no change is required, simply return the original file without saying anything more or less.`
    ...
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: `You are a batch processing bot. Your job is to update variables in the provided function only while maintaining the format. Be concise. You will output the entire updated file so it can be checked into git.`,
        },
